...
public class ExcelUtil {
     ...
     */
    public static <T> void exportExcel(Map<String,String> headers, Collection<T> dataset, OutputStream out,
    		String pattern, String className) throws ClassNotFoundException {
    	// 声明一个工作薄
    	HSSFWorkbook workbook = new HSSFWorkbook();
    	// 生成一个表格
    	HSSFSheet sheet = workbook.createSheet();
    	
    	write2Sheet(sheet, headers, dataset, className, pattern);
    	try {
    		workbook.write(out);
    	} catch (IOException e) {
     ...
     * @throws ClassNotFoundException 
     */
    private static <T> void write2Sheet(HSSFSheet sheet, Map<String,String> headers, Collection<T> dataset,String className,
    		String pattern) throws ClassNotFoundException {
        //时间格式默认"yyyy-MM-dd"
        ...
        String key = "";    //存放临时键变量
        // 遍历obj类数据，产生列数
    	List<FieldForSortting> columns = sortFieldByAnno(Class.forName(className));
    	int c= 0;   //标题列数
    	for(FieldForSortting i : columns) {
    		 Field field = i.getField();
    		 HSSFCell cell = row.createCell(c);
             HSSFRichTextString text = new HSSFRichTextString(headers.get(field.getName()));
